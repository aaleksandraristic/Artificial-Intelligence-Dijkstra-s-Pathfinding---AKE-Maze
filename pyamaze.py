...
class agent:
    ...
    def __init__(self, parentMaze, x=None, y=None, shape='square', goal=None, filled=False, footprints=False,
        ...
        """
        parent maze-->  The maze on which agent is placed.
        ...
        footprints-->   When the agent will move to some other cell, its footprints
...
class textLabel:
    ...
    def __init__(self, parentMaze, title, value):
        """
        parent maze-->  The maze on which Label will be displayed.
...
class maze:
    ...
    def CreateMaze(self, x=1, y=1, pattern=None, loopPercent=0, saveMaze=False, loadMaze=None,
        ...
        loadMaze--> Provide the CSV file to generate a desired maze
        theme--> Dark or Light
        """
        _stack = []
        _closed = []
        ...
        def removeWallBetween(cell1, cell2):
            """
        ...
        if not loadMaze:
            ...
            if loopPercent != 0:
                ...
                # remove blocks from the shortest path cells
                count = 0
                i = 0
                while count < count1:  # these many blocks to remove
                    if len(blockedNeighbours(pathCells[i])) > 0:
                        ...
                        if not isCyclic(cell, pathCells[i]):
                            removeWallBetween(cell, pathCells[i])
                            count += 1
                        i += 1
                    ...
                    while count < count2:  # these many blocks to remove
                        if len(blockedNeighbours(notPathCells[i])) > 0:
                            ...
                            if not isCyclic(cell, notPathCells[i]):
                                removeWallBetween(cell, notPathCells[i])
                                count += 1
                            i += 1
